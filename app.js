...
function applyFormulaCheckResult(scope,editor,result){var value=result.ok?"通过":"未通过",status=scope.querySelector(".formula-check-status");editor.classList.toggle("input-error",!result.ok);if(activeFormulaConfig){activeFormulaConfig.checkStatus=value;}if(status){status.textContent=formulaStatusText(value);status.className="formula-check-status "+(result.ok?"passed":"failed");}}
function bindFormulaEditors(){document.querySelectorAll(".formula-field-button,.formula-token").forEach(function(btn){btn.onclick=function(){insertFormulaToken(btn.dataset.editor,btn.dataset.token);};});document.querySelectorAll(".formula-editor").forEach(function(editor){editor.oninput=function(){resetActiveFormulaCheck();var container=editor.closest(".formula-panel,.annual-drawer-section-body"),panel=container&&container.querySelector(".formula-test-panel");if(panel){panel.classList.remove("show");panel.innerHTML="";}};});document.querySelectorAll(".formula-check").forEach(function(btn){btn.onclick=function(){var scope=btn.closest("#ruleFormulaConditionPanel,#ruleFormulaResult,#formulaConfigBody,.annual-drawer-section-body")||document,editor=scope.querySelector(".formula-editor");if(!editor){return;}var expected=formulaExpectedType(editor),result=validateFormulaText(editor.value,expected);applyFormulaCheckResult(scope,editor,result);showToast(result.ok?"公式检查通过":result.message);};});document.querySelectorAll(".formula-test").forEach(function(btn){btn.onclick=function(){var scope=btn.closest("#ruleFormulaConditionPanel,#ruleFormulaResult,#formulaConfigBody,.annual-drawer-section-body")||document,editor=scope.querySelector(".formula-editor");if(!editor){return;}var expected=formulaExpectedType(editor),result=validateFormulaText(editor.value,expected);applyFormulaCheckResult(scope,editor,result);if(!result.ok){showToast("公式检查未通过："+result.message);return;}var panel=scope.querySelector(".formula-test-panel");if(!panel){return;}panel.innerHTML=formulaTestPanelHtml(editor,expected);panel.classList.add("show");bindFormulaTestPanel(panel,editor,expected);showToast("公式检查通过，请填写示例数据");};});}
