...
var plans=[
  {id:"cn-main",name:"中国区假期方案",names:{zh_CN:"中国区假期方案",en_US:"China Leave Plan"},version:"00041",region:"中国区",personnel:["内部员工"],employee:["营销","职能","操作"],state:"启用",creator:"肖维",created:"2026-06-17 14:23",updater:"肖维",updated:"2026-07-24 18:25"},
  {id:"fr-main",name:"法国区假期方案",names:{zh_CN:"法国区假期方案",en_US:"France Leave Plan"},version:"00014",region:"法国区",personnel:["内部员工","外部人员"],employee:["营销","职能"],state:"启用",creator:"吴振兴",created:"2026-06-11 16:45",updater:"吴振兴",updated:"2026-07-18 10:16"},
  {id:"ca-legacy",name:"加拿大区历史方案",names:{zh_CN:"加拿大区历史方案",en_US:"Canada Legacy Leave Plan"},version:"00003",region:"加拿大区",personnel:["内部员工"],employee:["操作"],state:"停用",creator:"陈南",created:"2026-06-03 09:20",updater:"陈南",updated:"2026-07-08 14:05"}
];
var editingPlanId="";
...
var unifiedRules={
  ...
  bereavement:rule({entitlementMode:"按总可休时长控制",durationRules:[durationRule([condition("休假申请单·去世亲属身份","等于","配偶"),condition("休假申请单·去世亲属身份","等于","父母"),condition("休假申请单·去世亲属身份","等于","子女")],"3",{name:"配偶、父母或子女",logic:"满足任一条件（或）",unit:"天"})],usageMode:"必须一次性休完",continuousLeave:"是",attachmentEnabled:true,attachmentRules:[{basis:"所有申请",operator:"",value:"",materials:["死亡证明","亲属关系证明"]}],policyEnabled:true,policyNotice:"请在申请单选择去世亲属身份，可休时长按公司政策执行。"}),
...
function planScopeValues(plan){if(plan&&Array.isArray(plan.applyScopes)&&plan.applyScopes.length){return plan.applyScopes.slice();}var scopes=[];if(plan&&plan.personnel&&plan.personnel.length){scopes.push("人员类型");}if(plan&&plan.employee&&plan.employee.length){scopes.push("员工类型");}return scopes.length?scopes:["人员类型"];}
function syncPlanScopeFields(clearHidden){
  var scopes=selectedCheckboxValues("planApplyScopes"),personnelItem=document.getElementById("planPersonnelItem"),employeeItem=document.getElementById("planEmployeeItem"),showPersonnel=scopes.indexOf("人员类型")>-1,showEmployee=scopes.indexOf("员工类型")>-1;
...
function renderPlans(){
  ...
  var rows=plans.filter(function(p){return planNameMatches(p,name)&&(region==="全部"||planRegionValues(p).indexOf(region)>-1)&&(personnel==="全部"||p.personnel.indexOf(personnel)>-1)&&(employee==="全部"||p.employee.indexOf(employee)>-1)&&(state==="全部"||p.state===state);}).sort(function(a,b){return String(b.created).localeCompare(String(a.created))||String(b.updated).localeCompare(String(a.updated));});currentFilteredPlans=rows.slice();
...
function openPlanEditor(id,isNew,copySourceId,historyVersion){
  ...
  var title=viewingHistoricalVersion?planDisplayName(p)+" · 历史版本":isNew?"新增假期方案":planDisplayName(p);
  var mode=viewingHistoricalVersion?"查看历史版本":isNew?"新增假期方案":"编辑假期方案";
  var meta=viewingHistoricalVersion?'<span>方案版本：'+viewingHistoricalVersion+'</span><span>只读</span>':source?'<span>已复制“'+planDisplayName(source)+'”的配置，请重新填写方案名称；保存后生成新方案并启用</span>':isNew?'<span>首次保存后生成版本并启用</span>':'<span>方案版本：'+p.version+'</span><span>状态：'+p.state+'</span>';
...
function conditionOperatorsForField(field){var type=conditionFieldType(field);if(type==="text"){return ["等于","不等于","包含","不包含"];}if(type==="enum"){return ["等于","不等于"];}if(type==="date"){return ["等于","早于","早于等于","晚于","晚于等于"];}return ["等于","不等于","大于","大于等于","小于","小于等于"];}
...
function bindUnifiedForm(key,leave,ruleData){
  ...
  if(unit){var previousUnit=leave.unit;unit.onchange=function(){
    ...
    var minInput=document.getElementById("leaveMin"),maxInput=document.getElementById("leaveMax"),hasRequestLimit=String((minInput&&minInput.value)||"").trim()||String((maxInput&&maxInput.value)||"").trim(),hasDurationConfig=(ruleData.durationRules||[]).length||String(ruleData.durationFormula||"").trim(),hasAttachmentThreshold=hasUnitDependentAttachmentValue();
    if(nextUnit!==previousUnit&&(hasRequestLimit||hasDurationConfig||hasAttachmentThreshold)&&!window.confirm("修改计量单位后，单次最小/最大时长、可休时长规则、周期上限及按申请时长设置的附件阈值，需要按新单位重新填写。系统不会自动换算，日最大时长保持不变。是否继续？")){this.value=previousUnit;setTimeout(function(){leaveFormDirty=dirtyBefore;},0);return;}
    if(nextUnit!==previousUnit){ruleData.durationRules=[];ruleData.durationFormula="";ruleData.durationFormulaCheckStatus="未检测";clearUnitDependentAttachmentValues(ruleData);if(minInput){minInput.value="";}if(maxInput){maxInput.value="";}}
    ...
    ["leaveMin","leaveMax"].forEach(function(id){var input=document.getElementById(id),field=input&&input.closest(".business-field"),suffix=input&&input.closest(".input-with-suffix")&&input.closest(".input-with-suffix").querySelector(".input-suffix");if(suffix){suffix.textContent=leave.unit;}if(field){var tip=leave.unit==="天"?"按整张申请累计的有效休假天数校验，不按每日休假工时换算。":"按整张申请累计的休假小时数校验。",required=id==="leaveMin"?'<span class="required">*</span>':"";field.querySelector(".form-label").innerHTML=required+(id==="leaveMin"?"单次最小时长":"单次最大时长")+' <span class="hint" data-tip="'+tip+'">?</span>';}});
...
function validateLeaveForm(ruleData){
  ...
  if(!(leaveMin>0)){document.getElementById("leaveMin").classList.add("input-error");return "单次最小时长必须大于0";}
  if(leaveUnit==="天"&&!Number.isInteger(leaveMin)){document.getElementById("leaveMin").classList.add("input-error");return "计量单位为天时，单次最小时长必须为整数天";}
  if(leaveMax!==null&&(!(leaveMax>0)||leaveMax<leaveMin)){document.getElementById("leaveMax").classList.add("input-error");return "单次最大时长不得小于单次最小时长";}
  if(leaveUnit==="天"&&leaveMax!==null&&!Number.isInteger(leaveMax)){document.getElementById("leaveMax").classList.add("input-error");return "计量单位为天时，单次最大时长必须为整数天";}
