...
function renderLeaves(){
  ...
  document.getElementById("leaveRows").innerHTML=rows.length?rows.map(function(x){var r=unifiedRules[x.key]||defaultRule();return '<tr><td><b>'+x.name+'</b></td><td>'+x.unit+'</td><td>'+x.min+' 小时</td><td>'+(x.max?x.max+' 小时':'不限制')+'</td><td>'+x.calc+'</td><td>'+x.daily+' 小时</td><td>'+x.roundBase+(x.roundBase==="—"?'':' 小时')+'</td><td>'+x.round+'</td><td><span class="control-pill">'+entitlementModeLabel(r.entitlementMode)+'</span></td><td>'+x.paid+'</td><td><div class="row-actions"><button class="btn-text edit-leave" data-key="'+x.key+'">编辑</button><button class="btn-text delete-action delete-leave" data-key="'+x.key+'" data-name="'+x.name+'">删除</button></div></td></tr>';}).join(""):'<tr><td class="table-empty" colspan="11">没有符合条件的休假配置</td></tr>';
...
function conditionNeedsValue(){return true;}
...
function attachmentRulesContentHtml(rows,leave){if(!(rows||[]).length){return '<div class="rule-empty-state empty-attachment-rules"><div><span class="rule-empty-badge">待完善</span>已启用附件要求，请至少添加一条附件规则。</div><button type="button" class="btn add-attachment-rule">＋ 添加附件规则</button></div>';}return '<div class="table-wrap"><table class="business-table attachment-rule-table"><thead><tr><th>判断依据 <span class="hint" data-tip="本次申请时长取提交前试算结果；休假日期跨度包含开始日和结束日，按自然日计算。">?</span></th><th>什么时候需要</th><th>需要上传的附件类型</th><th>提交要求</th><th>操作</th></tr></thead><tbody id="attachmentRuleRows">'+rows.map(function(row){return attachmentRuleRowHtml(row,leave);}).join("")+'</tbody></table></div><div class="attachment-match-note">同一次申请命中多条规则时，每条规则都需要满足；重复的附件类型只需上传一次。</div><div class="sub-action"><button type="button" class="btn add-attachment-rule">＋ 添加附件规则</button></div>';}
