...
function ruleResultGroupTitle(mode,fallback){if(mode==="按周期限制使用"){return fallback?"其他情况可使用多少":"2. 每个周期内可以怎么使用";}return fallback?"其他情况可以休多少":"2. 符合条件时可以休多少";}
function openDurationRuleEditor(index,key,leave,ruleData,scopeType){var mode=ruleData.entitlementMode,editorLeave=Object.assign({},leave,{unit:fieldValue("leaveUnit",leave.unit||"小时")}),r=index>=0?JSON.parse(JSON.stringify(ruleData.durationRules[index])):newDurationRuleFromContext(mode,editorLeave,ruleData,scopeType),fallback=ruleScopeType(r)==="all";r.conditionMode="常规条件";r.resultMode="直接填写";activeDurationRuleEditor={index:index,key:key,leave:editorLeave,ruleData:ruleData,mode:mode,source:r};durationRuleEditorDirty=false;document.getElementById("ruleEditorTitle").textContent=index>=0?(fallback?"编辑其他情况（兜底）":"编辑条件规则"):(fallback?"添加其他情况（兜底）":"新增条件规则");document.getElementById("saveDurationRule").textContent="确定";var body=document.getElementById("ruleEditorBody"),conditionGroup=fallback?'<div class="fallback-editor-note">当前面的条件规则均未命中时，使用本条结果。系统会将本条固定在规则列表最后。</div>':'<div class="rule-editor-group"><div class="rule-editor-group-title">1. 时长条件</div><div id="ruleEditorConditions">'+durationConditionEditorHtml(r)+'</div>'+conditionSourceBoxHtml(r)+'</div>';body.innerHTML=conditionGroup+'<div class="rule-editor-group"><div class="rule-editor-group-title">'+ruleResultGroupTitle(mode,fallback)+'</div>'+durationRuleResultEditorHtml(mode,r,editorLeave)+'</div>';document.getElementById("ruleEditorMask").classList.add("show");bindDurationRuleEditor();body.oninput=function(){durationRuleEditorDirty=true;};body.onchange=function(){durationRuleEditorDirty=true;};}
