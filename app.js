...
function annualFormulaDrawerHtml(){return `
  ...
  <section class="annual-drawer-section"><div class="annual-drawer-section-title">普通年度公式</div><div class="annual-drawer-section-body"><textarea class="formula-editor" id="annualFormulaText">取标准额度() × 取自然年实际在职天数(入职日期) ÷ 取自然年总天数()</textarea><div class="annual-formula-meta"><span>返回值：截至计算日累计应得额度（小时）</span><span>校验状态：公式正确</span></div><div class="section-result"><b>为什么使用“实际在职天数”：</b>预发＋每日的计算周期截止到当天；跨档时系统还会把周期按跨档日拆段。该函数会按每个实际计算区间取数，普通年度不会提前计入未来日期，跨档年度也不会重复计算全年天数。</div></div></section>
...
function openAnnualDrawer(title,body,context){var drawer=document.getElementById("drawer"),confirm=document.getElementById("confirmRules");drawer.classList.remove("narrow","option-guide","help-mode");drawer.dataset.context=context||"annual";document.getElementById("drawerTitle").textContent=title;document.getElementById("drawerBody").innerHTML=body;confirm.textContent="保存配置";document.getElementById("drawerMask").classList.add("show");}
...
function validateFormulaText(formula,expected){
  var text=String(formula||"").trim();if(!text){return {ok:false,message:expected==="condition"?"请填写时长条件公式":"请填写可休时长公式"};}
  ...
  return {ok:true,message:expected==="condition"?"条件公式检查通过，将返回满足或不满足":"公式检查通过，将返回可休时长"};
}
...
function bindFormulaEditors(){document.querySelectorAll(".formula-field-button,.formula-token").forEach(function(btn){btn.onclick=function(){insertFormulaToken(btn.dataset.editor,btn.dataset.token);};});document.querySelectorAll("#formulaConfigBody .formula-editor").forEach(function(editor){editor.oninput=function(){resetActiveFormulaCheck();};});document.querySelectorAll(".formula-check").forEach(function(btn){btn.onclick=function(){var scope=btn.closest("#ruleFormulaConditionPanel,#ruleFormulaResult,#formulaConfigBody,.annual-drawer-section-body")||document,editor=scope.querySelector(".formula-editor");if(!editor){return;}editor.classList.remove("input-error");var expected=/Condition|qualification/i.test(editor.id)?"condition":"duration",result=validateFormulaText(editor.value,expected),status=scope.querySelector(".formula-check-status");if(!result.ok){editor.classList.add("input-error");}if(activeFormulaConfig){activeFormulaConfig.checkStatus=result.ok?"通过":"未通过";}if(status){status.textContent=result.ok?"通过":"未通过";status.className="formula-check-status "+(result.ok?"passed":"failed");}showToast(result.message);};});document.querySelectorAll(".formula-test").forEach(function(btn){btn.onclick=function(){var scope=btn.closest("#ruleFormulaConditionPanel,#ruleFormulaResult,#formulaConfigBody,.annual-drawer-section-body")||document,editor=scope.querySelector(".formula-editor");if(!editor){return;}var expected=/Condition|qualification/i.test(editor.id)?"condition":"duration",result=validateFormulaText(editor.value,expected);if(!result.ok){editor.classList.add("input-error");showToast(result.message);return;}editor.classList.remove("input-error");showToast("已完成示例试算");};});}
