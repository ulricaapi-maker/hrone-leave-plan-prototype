...
var plans=[
  {id:"cn-main",name:"中国区假期方案",version:"00041",region:"中国区",personnel:["内部员工"],employee:["营销","职能","操作"],state:"启用",creator:"肖维",created:"2026-06-17 14:23",updater:"肖维",updated:"2026-07-24 18:25"},
  {id:"fr-main",name:"法国区假期方案",version:"00014",region:"法国区",personnel:["内部员工","外部人员"],employee:["营销","职能"],state:"启用",creator:"吴振兴",created:"2026-06-11 16:45",updater:"吴振兴",updated:"2026-07-18 10:16"},
  {id:"ca-legacy",name:"加拿大区历史方案",version:"00003",region:"加拿大区",personnel:["内部员工"],employee:["操作"],state:"停用",creator:"陈南",created:"2026-06-03 09:20",updater:"陈南",updated:"2026-07-08 14:05"}
];
var editingPlanId="";
...
var exportPlanFields=["方案名称","当前版本","划分区域","人员类型","员工类型","状态","创建人","创建时间","更新人","更新时间"];

var fieldCatalog=[
  {group:"员工档案",field:"员工档案·性别",values:["女","男"],note:"取休假开始日有效的员工档案。"},
  {group:"员工档案",field:"员工档案·婚姻状况",values:["已婚","未婚","离异","丧偶"],note:"取休假开始日有效的婚姻状况。"},
  {group:"员工档案",field:"员工档案·员工类型",values:["营销","职能","操作"],note:"取休假开始日有效的员工类型。"},
  {group:"员工任职信息",field:"员工任职信息·人员状态",values:["正式","试用"],note:"取额度计算日有效任职记录中的人员状态。"},
  {group:"员工任职信息",field:"员工任职信息·转正状态",values:["已转正","试用期"],note:"取休假开始日有效的任职状态。"},
...
var unifiedRules={
  ...
  sick:rule({entitlementMode:"不限制可休总时长",conditionEnabled:true,conditions:[condition("员工任职信息·转正状态","等于","已转正")],attachmentEnabled:true,attachmentRules:[{basis:"本次申请时长",operator:"大于",value:"1",materials:["诊断证明","病历","医疗费用单据"]}],policyEnabled:true,policyNotice:"请按要求提供医疗材料，具体病假政策以员工适用规则为准。"}),
  ...
  bereavement:rule({entitlementMode:"按总可休时长控制",durationRules:[durationRule([condition("休假申请单·去世亲属身份","属于","配偶、父母、子女")],"3",{name:"配偶、父母或子女",unit:"天"})],usageMode:"必须一次性休完",continuousLeave:"是",attachmentEnabled:true,attachmentRules:[{basis:"所有申请",operator:"",value:"",materials:["死亡证明","亲属关系证明"]}],policyEnabled:true,policyNotice:"请在申请单选择去世亲属身份，可休时长按公司政策执行。"}),
  womensDay:rule({entitlementMode:"按周期限制使用",durationRules:[durationRule([condition("员工档案·性别","等于","女"),condition("系统计算·当年3月8日是否周末","等于","否")],"",{name:"女性员工每年一次",kind:"periodic",cycle:"自然年",limitType:"申请次数＋单次时长",maxTimes:"1",singleDuration:"4",unit:"小时"})],leaveDateEnabled:true,leaveDateRules:[{target:"休假开始日期",relation:"不得早于",reference:"系统计算·当年3月8日",offset:"0",unit:"自然日"},{target:"休假结束日期",relation:"不得晚于",reference:"系统计算·当年3月31日",offset:"0",unit:"自然日"}],policyEnabled:true,policyNotice:"当年3月8日不是星期六或星期日时，符合条件的女性员工可在3月8日至3月31日申请1次三八妇女节假，每次4小时；如3月8日为星期六或星期日（含调休补班），当年不享受。"}),
...
var boundary={
  ...
  business:{title:"公出/出差方案",text:"维护公出和出差的计量及考勤规则。",chips:["公出","出差","考勤规则"]}
};
var editingLeaveKey="";
...
var leaveFilterConfig={
  ...
  min:{id:"leaveFilterMin",label:"单次最小时长不低于",type:"number",placeholder:"小时"},
  max:{id:"leaveFilterMax",label:"单次最大时长不高于",type:"number",placeholder:"小时"},
  ...
  control:{id:"leaveFilterControl",label:"可休时长控制",options:["全部","按可用额度控制","按政策条件确定可休时长","按周期设置可休上限","不设置累计上限"]},
...
function renderLeaves(){
  ...
  var rows=leaves.filter(function(x){var r=unifiedRules[x.key]||defaultRule(),controlLabel=entitlementModeLabel(r.entitlementMode),baseLabel=x.roundBase==="—"?"不适用":x.roundBase,roundLabel=x.round==="—"?"不适用":x.round;return (!name||x.name.indexOf(name)>-1)&&(unit==="全部"||x.unit===unit)&&(calc==="全部"||x.calc===calc)&&(control==="全部"||controlLabel===control)&&(paid==="全部"||x.paid===paid)&&(!minFilter||Number(x.min)>=Number(minFilter))&&(!maxFilter||(x.max&&Number(x.max)<=Number(maxFilter)))&&(!dailyFilter||Number(x.daily)<=Number(dailyFilter))&&(roundBase==="全部"||baseLabel===roundBase)&&(round==="全部"||roundLabel===round);});
  document.getElementById("leaveRows").innerHTML=rows.length?rows.map(function(x){var r=unifiedRules[x.key]||defaultRule(),requestUnit=x.unit==="天"?"小时":x.unit;return '<tr><td><b>'+x.name+'</b></td><td>'+x.unit+'</td><td>'+x.min+' '+requestUnit+'</td><td>'+(x.max?x.max+' '+requestUnit:'不限制')+'</td><td>'+x.calc+'</td><td>'+x.daily+' 小时</td><td>'+x.roundBase+(x.roundBase==="—"?'':' 小时')+'</td><td>'+x.round+'</td><td><span class="control-pill">'+entitlementModeLabel(r.entitlementMode)+'</span></td><td>'+x.paid+'</td><td><div class="row-actions"><button class="btn-text edit-leave" data-key="'+x.key+'">编辑</button><button class="btn-text delete-action delete-leave" data-key="'+x.key+'" data-name="'+x.name+'">删除</button></div></td></tr>';}).join(""):'<tr><td class="table-empty" colspan="11">没有符合条件的休假配置</td></tr>';
  document.querySelectorAll(".edit-leave").forEach(function(btn){btn.onclick=function(){openLeaveForm(btn.dataset.key);};});
  document.querySelectorAll(".delete-leave").forEach(function(btn){btn.onclick=function(){if(["annual","comp","sick"].indexOf(btn.dataset.key)>-1){openConfirm("暂不能删除"+btn.dataset.name,"“"+btn.dataset.name+"”已配置专属额度规则，不能直接从当前方案删除。<div class=\"confirm-note\">如不再使用，请先处理对应额度配置及存量员工数据，再按停用或迁移方案处理。</div>",null);return;}openConfirm("删除休假配置","确定从当前方案版本中删除“"+btn.dataset.name+"”吗？<div class=\"confirm-note\">不会删除休假类型主数据，也不会影响历史版本和历史申请。</div>",function(){leaves=leaves.filter(function(x){return x.key!==btn.dataset.key;});renderLeaves();planFormDirty=true;showToast("已从当前方案移除"+btn.dataset.name);});};});
...
function formulaActivationIssues(){var issues=[];leaves.forEach(function(leave){var r=unifiedRules[leave.key]||defaultRule();if(r.durationMode==="统一公式"&&String(r.durationFormula||"").trim()&&normalizedFormulaStatus(r.durationFormula,r.durationFormulaCheckStatus)!=="通过"){issues.push(leave.name+"的可休时长公式未通过检测");}if(r.conditionEnabled&&r.conditionMode==="公式配置"&&String(r.conditionFormula||"").trim()&&normalizedFormulaStatus(r.conditionFormula,r.conditionFormulaCheckStatus)!=="通过"){issues.push(leave.name+"的申请资格公式未通过检测");}});return issues;}
function renderPlans(){
  ...
  var rows=plans.filter(function(p){return (!name||p.name.indexOf(name)>-1)&&(region==="全部"||p.region===region)&&(personnel==="全部"||p.personnel.indexOf(personnel)>-1)&&(employee==="全部"||p.employee.indexOf(employee)>-1)&&(state==="全部"||p.state===state);});currentFilteredPlans=rows.slice();
  ...
  document.getElementById("planRows").innerHTML=rows.length?rows.map(function(p){return '<tr><td><b>'+p.name+'</b></td><td><button class="version-link" data-id="'+p.id+'">'+p.version+'</button></td><td>'+p.region+'</td><td>'+formatList(p.personnel)+'</td><td>'+formatList(p.employee)+'</td><td><span class="status '+(p.state==="停用"?'disabled':'')+'">'+p.state+'</span></td><td>'+p.creator+'</td><td>'+p.created+'</td><td>'+p.updater+'</td><td>'+p.updated+'</td><td><div class="row-actions"><button class="btn-text edit-plan" data-id="'+p.id+'">编辑</button><button class="btn-text copy-plan" data-id="'+p.id+'">复制</button><button class="btn-text toggle-plan" data-id="'+p.id+'">'+(p.state==="启用"?"停用":"启用")+'</button></div></td></tr>';}).join(""):'<tr><td class="table-empty" colspan="11">没有符合条件的假期方案</td></tr>';
...
function bindEnumMultiSelect(id){var box=document.getElementById(id);if(!box){return;}var trigger=box.querySelector(".enum-select-trigger"),menu=box.querySelector(".enum-select-menu");trigger.onclick=function(e){e.stopPropagation();var willOpen=!box.classList.contains("open");closeEnumMultiSelects(box);box.classList.toggle("open",willOpen);menu.hidden=!willOpen;trigger.setAttribute("aria-expanded",String(willOpen));};menu.onclick=function(e){e.stopPropagation();};box.querySelectorAll('input[type="checkbox"]').forEach(function(input){input.onchange=function(){updateEnumMultiSelect(id);planFormDirty=true;};});updateEnumMultiSelect(id);}
...
function openPlanEditor(id,isNew,copySourceId,historyVersion){
  ...
  var p=isNew?(source?JSON.parse(JSON.stringify(source)):{name:"",version:"保存后生成",region:"中国区",personnel:["内部员工"],employee:[],state:"启用"}):plans.find(function(x){return x.id===id;});
  if(source){p.name="";p.version="保存后生成";p.state="启用";}
  var title=viewingHistoricalVersion?p.name+" · 历史版本":isNew?"新增假期方案":p.name;
  var mode=viewingHistoricalVersion?"查看历史版本":isNew?"新增假期方案":"编辑假期方案";
  var meta=viewingHistoricalVersion?'<span>方案版本：'+viewingHistoricalVersion+'</span><span>只读</span>':source?'<span>已复制“'+source.name+'”的配置，请重新填写方案名称；保存后生成新方案并启用</span>':isNew?'<span>首次保存后生成版本并启用</span>':'<span>方案版本：'+p.version+'</span><span>状态：'+p.state+'</span>';
  document.getElementById("planEditorTitle").textContent=title;document.getElementById("editorModeText").textContent=mode;
  document.getElementById("planEditorMeta").innerHTML=meta;document.getElementById("planName").value=p.name;document.getElementById("planRegion").value=p.region;document.getElementById("planVersionDisplay").textContent=viewingHistoricalVersion||p.version;setCheckboxValues("planPersonnelTypes",p.personnel);setCheckboxValues("planEmployeeTypes",p.employee);closeEnumMultiSelects();planFormDirty=false;showView("edit");
}
function collectPlanForm(){return {name:document.getElementById("planName").value.trim(),region:document.getElementById("planRegion").value,personnel:selectedCheckboxValues("planPersonnelTypes"),employee:selectedCheckboxValues("planEmployeeTypes")};}
function validatePlanForm(form){if(!form.name){return "请填写方案名称";}if(!form.personnel.length){return "请至少选择一种人员类型";}if(!form.employee.length){return "请至少选择一种员工类型";}return "";}
function savePlan(){
  var form=collectPlanForm(),message=validatePlanForm(form);if(message){showToast(message);return;}
  var formulaIssues=formulaActivationIssues(),targetState=formulaIssues.length?"停用":"启用";
  if(editingPlanIsNew){var plan={id:"plan-"+Date.now(),name:form.name,version:"00001",region:form.region,personnel:form.personnel,employee:form.employee,state:targetState,creator:"当前用户",created:"2026-07-25 10:00",updater:"当前用户",updated:"2026-07-25 10:00"};plans.unshift(plan);editingPlanId=plan.id;editingPlanIsNew=false;showToast(formulaIssues.length?"方案已保存为停用；"+formulaIssues[0]:(editingPlanCopySource?"复制方案已保存并启用，版本为00001":"方案已保存并启用，版本为00001"));editingPlanCopySource="";}
  else{var current=plans.find(function(x){return x.id===editingPlanId;}),next=String(parseInt(current.version,10)+1).padStart(5,"0");Object.assign(current,form,{version:next,state:formulaIssues.length?"停用":current.state,updater:"当前用户",updated:"2026-07-25 10:00"});showToast(formulaIssues.length?"方案已保存并停用；"+formulaIssues[0]:"方案已保存并生成版本"+next);}
...
function openConfirm(title,body,action){document.getElementById("confirmTitle").textContent=title;document.getElementById("confirmBody").innerHTML=body;pendingConfirmAction=action;document.getElementById("confirmMask").classList.add("show");}
function closeConfirm(){document.getElementById("confirmMask").classList.remove("show");pendingConfirmAction=null;}
function openExportPlanSettings(){document.querySelectorAll("#exportPlanFields input").forEach(function(input){input.checked=exportPlanFields.indexOf(input.value)>-1;});document.getElementById("exportSettingsMask").classList.add("show");}
function closeExportPlanSettings(){document.getElementById("exportSettingsMask").classList.remove("show");}
function saveExportPlanSettings(){var selected=Array.from(document.querySelectorAll("#exportPlanFields input:checked")).map(function(input){return input.value;});if(!selected.length){showToast("请至少选择一个导出字段");return;}exportPlanFields=selected;closeExportPlanSettings();showToast("导出字段已保存");}
function exportCurrentPlans(){if(!currentFilteredPlans.length){showToast("当前查询结果没有可导出的方案");return;}showToast("已按当前查询结果导出"+currentFilteredPlans.length+"条方案（"+exportPlanFields.length+"个字段）");}
function closeVersionPopover(){document.getElementById("versionPopover").classList.remove("show");}
function openVersionHistory(id,anchor){var p=plans.find(function(x){return x.id===id;}),latest=parseInt(p.version,10),versions=[];for(var i=0;i<Math.min(5,latest);i++){versions.push(String(latest-i).padStart(5,"0"));}var popover=document.getElementById("versionPopover");popover.innerHTML='<div class="version-popover-head"><span>'+p.name+' · 历史版本</span><button class="close" id="closeVersionPopover">×</button></div><div class="table-wrap"><table><thead><tr><th>序号</th><th>方案版本</th><th>最近修改人</th><th>最近修改时间</th></tr></thead><tbody>'+versions.map(function(v,index){return '<tr><td>'+(index+1)+'</td><td><button class="version-link history-version" data-version="'+v+'">'+v+'</button></td><td>'+(index===0?p.updater:p.creator)+'</td><td>'+(index===0?p.updated:p.created)+'</td></tr>';}).join("")+'</tbody></table></div><div class="pagination"><span>共'+versions.length+'条</span><span class="page-current">1</span><span>20条/页</span></div>';popover.classList.add("show");positionFloatingPopover(popover,anchor);document.getElementById("closeVersionPopover").onclick=closeVersionPopover;popover.querySelectorAll(".history-version").forEach(function(btn){btn.onclick=function(){var version=btn.dataset.version;closeVersionPopover();openPlanEditor(id,false,"",version);showToast("正在查看历史版本"+version);};});}
...
function conditionFieldType(field){var meta=findField(field);if(field==="休假申请单·孕周"||/月数|天数|时长/.test(field)){return "number";}if(/日期|预产期/.test(field)){return "date";}if(meta&&meta.values){return "enum";}return "text";}
function conditionOperatorsForField(field){var type=conditionFieldType(field);if(type==="text"){return ["等于","不等于","包含","不包含","已填写","未填写"];}if(type==="enum"){return ["等于","不等于","属于","不属于","已填写","未填写"];}if(type==="date"){return ["等于","早于","早于等于","晚于","晚于等于","已填写","未填写"];}return ["等于","不等于","大于","大于等于","小于","小于等于","已填写","未填写"];}
function conditionNeedsValue(operator){return operator!=="已填写"&&operator!=="未填写";}
function fieldValueControl(field,value){var meta=findField(field),type=conditionFieldType(field);if(field==="休假申请单·孕周"){var p=gestationParts(value);return '<span class="condition-value gestation-control" data-value="'+(value||"")+'"><input class="form-input gestation-week" type="number" min="0" value="'+p.weeks+'"><span>周</span><input class="form-input gestation-day" type="number" min="0" max="6" value="'+p.days+'"><span>天</span></span>';}if(meta&&meta.values){return '<select class="form-select condition-value">'+optionListHtml(meta.values.concat([value]),value)+'</select>';}return '<input class="form-input condition-value" type="'+(type==="date"?"date":type==="number"?"number":"text")+'" value="'+(value||"")+'" placeholder="请输入或选择条件值">';}
function conditionValueCellHtml(field,value,operator){return conditionNeedsValue(operator)?fieldValueControl(field,value):'<span class="condition-presence-value">无需填写具体值</span><input class="condition-value" type="hidden" value="">';}
...
function ruleConditionSummary(r){var conditions=r.conditions||[];if(ruleScopeType(r)==="all"){return "其他情况";}if(r.conditionMode==="条件公式"){return String(r.conditionFormula||"条件公式").replace(/\s+/g," ").trim();}var joiner=r.logic==="满足任一条件（或）"?" 或 ":" 且 ";return conditions.map(function(c){return simpleFieldName(c.field)+conditionOperatorText(c.operator)+(conditionNeedsValue(c.operator)?c.value:"");}).join(joiner);}
function durationRuleName(r,index){if(r.name){return r.name;}var conditions=r.conditions||[];if(r.conditionMode==="条件公式"){return "高级公式规则"+(index+1);}if(conditions.length===1){var c=conditions[0];if(c.field==="员工任职信息·工作城市"){return c.value||"地区规则";}return simpleFieldName(c.field)+conditionOperatorText(c.operator)+(c.value||"");}return conditions.length?"组合条件"+(index+1):"统一时长";}
...
function bindQualificationFormulaActions(){document.querySelectorAll(".configure-qualification-formula").forEach(function(btn){btn.onclick=function(){openQualificationFormula(false);};});document.querySelectorAll(".view-qualification-formula").forEach(function(btn){btn.onclick=function(){openQualificationFormula(true);};});document.querySelectorAll(".clear-qualification-formula").forEach(function(btn){btn.onclick=function(){if(!window.confirm("确定清除已配置的高级公式吗？")){return;}activeLeaveRuleDraft.conditionFormula="";activeLeaveRuleDraft.conditionFormulaCheckStatus="未检测";markLeaveFormDirty();renderQualificationFormulaSummary();};});}
function openQualificationFormula(readonly){var value=(activeLeaveRuleDraft&&activeLeaveRuleDraft.conditionFormula)||"",status=(activeLeaveRuleDraft&&activeLeaveRuleDraft.conditionFormulaCheckStatus)||"未检测";activeFormulaConfig={readonly:readonly,kind:"qualification",checkStatus:status,originalValue:value};document.getElementById("formulaConfigTitle").textContent=readonly?"查看申请资格公式":"配置申请资格公式";document.getElementById("formulaConfigBody").innerHTML=formulaEditorHtml("qualificationFormulaDraft",value,"",status);document.getElementById("saveFormulaConfig").style.display=readonly?"none":"inline-flex";document.getElementById("formulaConfigMask").classList.add("show");bindFormulaEditors();if(readonly){var textarea=document.getElementById("qualificationFormulaDraft");textarea.readOnly=true;document.querySelectorAll("#formulaConfigBody .formula-toolbar,#formulaConfigBody .formula-actions,#formulaConfigBody .field-panel,#formulaConfigBody .formula-check-row").forEach(function(el){el.style.display="none";});document.querySelector("#formulaConfigBody .advanced-layout").style.gridTemplateColumns="1fr";}}
function closeFormulaConfig(){document.getElementById("formulaConfigMask").classList.remove("show");activeFormulaConfig=null;}
function durationFormulaSummaryHtml(leave,rule){var formula=String(rule.durationFormula||"").trim();if(!formula){return '<div class="duration-formula-card"><div class="duration-formula-card-head"><div><div class="duration-formula-title">尚未配置统一公式</div><div class="duration-formula-desc">一个公式同时表达判断条件和可休结果，最终返回可休时长，单位为'+leave.unit+'。</div></div><button type="button" class="btn btn-primary configure-duration-formula">配置公式</button></div></div>';}return '<div class="duration-formula-card"><div class="duration-formula-card-head"><div><div class="duration-formula-title">统一公式已配置 '+formulaStatusTag(formula,rule.durationFormulaCheckStatus)+'</div><div class="duration-formula-desc">最终返回可休时长，单位为'+leave.unit+'。</div></div><div class="duration-formula-actions"><button type="button" class="btn-text view-duration-formula">查看</button><button type="button" class="btn-text configure-duration-formula">修改</button><button type="button" class="btn-text clear-duration-formula">清除</button></div></div><div class="duration-formula-preview">'+escapeRuleHtml(formula)+'</div></div>';}
function openDurationFormula(readonly,leave,rule){var value=rule.durationFormula||"",status=rule.durationFormulaCheckStatus||"未检测";activeFormulaConfig={readonly:readonly,kind:"duration",leave:leave,rule:rule,checkStatus:status,originalValue:value};document.getElementById("formulaConfigTitle").textContent=readonly?"查看可休时长公式":"配置可休时长公式";document.getElementById("formulaConfigBody").innerHTML=formulaEditorHtml("durationFormulaDraft",value,"在同一个公式中写明判断条件与可休结果；公式最终返回一个可休时长数值，单位为“"+leave.unit+"”。",status);document.getElementById("saveFormulaConfig").style.display=readonly?"none":"inline-flex";document.getElementById("formulaConfigMask").classList.add("show");bindFormulaEditors();if(readonly){var textarea=document.getElementById("durationFormulaDraft");textarea.readOnly=true;document.querySelectorAll("#formulaConfigBody .formula-toolbar,#formulaConfigBody .field-panel,#formulaConfigBody .formula-check-row").forEach(function(el){el.style.display="none";});document.querySelector("#formulaConfigBody .advanced-layout").style.gridTemplateColumns="1fr";}}
function entitlementDetailHtml(key,leave,rule){var mode=rule.entitlementMode;if(mode==="按可用余额控制"||mode==="不限制可休总时长"){return '';}var formulaMode=rule.durationMode==="统一公式";return '<div class="duration-config-head"><div><div class="form-label"><span class="required">*</span>可休时长配置</div><div class="duration-config-mode"><button type="button" class="duration-config-option'+(formulaMode?'':' active')+'" data-duration-mode="规则表">常规条件</button><button type="button" class="duration-config-option'+(formulaMode?' active':'')+'" data-duration-mode="统一公式">高级公式</button></div></div></div><div id="durationConfigContent">'+(formulaMode?durationFormulaSummaryHtml(leave,rule):durationRulesSummaryTableHtml(mode,rule.durationRules,leave))+'</div>';}
function unifiedEntitlementHtml(key,leave,rule){return businessSectionHtml("3. 可休时长控制","",entitlementModeSelectHtml(key,rule.entitlementMode)+'<div id="entitlementDetail">'+entitlementDetailHtml(key,leave,rule)+'</div>');}

function unifiedApplicationHtml(leave,rule){var conditionsOn=rule.conditionEnabled,conditionMode=rule.conditionMode||"常规条件",standard=conditionMode!=="公式配置",conditionBody=configModeSwitchHtml("conditionMode",standard?"常规条件":"公式配置","常规条件")+'<div id="conditionStandardPanel"'+(standard?'':' style="display:none"')+'>'+conditionSetHtml(rule.conditions,rule.conditionLogic,"eligibility-condition-list",null,true)+'</div><div id="conditionFormulaPanel"'+(standard?' style="display:none"':'')+'>'+qualificationFormulaSummaryHtml(rule.conditionFormula,rule.conditionFormulaCheckStatus)+'</div>';var qualifications='<div class="linked-box rule-module"><div class="attachment-title"><button type="button" class="switch-toggle inline-rule-toggle '+(conditionsOn?'on':'')+'" data-target="applicationConditionBody" aria-pressed="'+conditionsOn+'"></button><span>设置申请资格</span></div><div id="applicationConditionBody"'+(conditionsOn?'':' style="display:none"')+'>'+conditionBody+'</div></div>';return businessSectionHtml("4. 申请资格","按员工档案、任职信息或系统计算结果判断能否申请；入职等待期也在这里配置。",qualifications);}
...
function dateRulesInnerHtml(rows){if(!(rows||[]).length){return '<div class="rule-empty-state empty-date-rules"><div><span class="rule-empty-badge">待完善</span>已启用休假日期范围，请至少添加一条日期规则。</div><button type="button" class="btn add-date-rule">＋ 添加日期规则</button></div>';}return '<div class="date-rule-header"><span><i class="required">*</i>校验哪个休假日期</span><span><i class="required">*</i>必须满足</span><span><i class="required">*</i>参照日期 <span class="hint" data-tip="只能选择系统已保存的日期；材料中未结构化登记的日期由审批人核验，不能自动校验。">?</span></span><span><i class="required">*</i>时间范围 <span class="hint" data-tip="自然日连续计算日历日期；工作日按企业工作日历计算；日历月、日历年按对应日期偏移。">?</span></span><span>操作</span></div><div class="date-rule-list">'+rows.map(dateRuleRowHtml).join("")+'</div><div class="sub-action"><button type="button" class="btn add-date-rule">＋ 添加日期规则</button></div>';}
function dateRulesHtml(rows){return '<div id="dateRulesEditor">'+dateRulesInnerHtml(rows)+'</div>';}
function unifiedLeaveDateHtml(rule){var on=rule.leaveDateEnabled;return businessSectionHtml("5. 休假日期范围","",'<div class="linked-box rule-module"><div class="attachment-title"><button type="button" class="switch-toggle inline-rule-toggle '+(on?'on':'')+'" data-target="leaveDateRuleBody" aria-pressed="'+on+'"></button><span>设置休假日期范围</span></div><div id="leaveDateRuleBody"'+(on?'':' style="display:none"')+'>'+dateRulesHtml(rule.leaveDateRules)+'</div></div>');}
function intervalUnitOptions(unit){return unit==="小时"?["小时","自然日","工作日","周","月"]:["自然日","工作日","周","月"];}
function unifiedIntervalHtml(leave,rule){var on=!!rule.intervalEnabled,options=intervalUnitOptions(leave.unit),current=options.indexOf(rule.intervalUnit)>-1?rule.intervalUnit:"自然日";return businessSectionHtml("7. 申请间隔","",'<div class="linked-box rule-module"><div class="attachment-title"><button type="button" class="switch-toggle inline-rule-toggle '+(on?'on':'')+'" data-target="intervalRuleBody" aria-pressed="'+on+'"></button><span>限制相邻两次同类休假的间隔</span></div><div id="intervalRuleBody"'+(on?'':' style="display:none"')+'>'+businessFieldsHtml([{label:"上一笔有效休假结束后至少间隔",value:rule.intervalValue||"",id:"intervalValue",required:true,type:"number",min:"0",tip:"只比较有效的同类休假；驳回、撤回、终止及已全部销假的单据不计入。"},{label:"间隔单位",value:current,options:options,id:"intervalUnit",required:true,tip:"小时只用于有明确起止时间的小时制休假；周按7个自然日计算，月按对应日历日期计算。"}])+'</div></div>');}
function unifiedUsageModeHtml(rule){var value=rule.usageMode==="必须一次性休完"?"必须一次申请完":"可分次申请";return businessSectionHtml("8. 可休时长使用方式","",businessFieldsHtml([{label:"可休时长使用方式",value:value,options:["可分次申请","必须一次申请完"],id:"mustCompleteOnce",required:true,direct:true,wideChoice:true,tip:"只控制一项可休权益是否可以拆成多张申请，不决定休假日期是否连续。"}]));}
function unifiedOtherHtml(leave){var prerequisite=arrayValue(leave.prerequisite),options=leaves.filter(function(x){return x.name!==leave.name;}).map(function(x){return x.name;}),strongChecked=leave.strong==="是";return businessSectionHtml("9. 前置假要求","",'<span class="form-label">前置假（可多选）</span>'+checkboxGroupHtml("preposHolidayGroup",options,prerequisite)+'<div class="strong-control-row'+(prerequisite.length?'':' is-disabled')+'" id="preleaveControlRow"><label class="checkbox-option strong-control-option"><input type="checkbox" id="unifiedStrongControl"'+(strongChecked?' checked':'')+(prerequisite.length?'':' disabled')+'>强管控</label><span class="hint" data-tip="强管控：只有前置假余额小于等于0时，才能申请当前假别。未勾选为弱管控：前置假余额不足以覆盖本次申请时，即可申请当前假别。">?</span></div>',"","preleave-section");}
...
function attachmentRuleRowHtml(row,leave){row=row||{basis:"所有申请",operator:"大于",value:"",materials:[],requirement:"全部提供"};var conditional=row.basis!=="所有申请",unit=row.basis==="休假日期跨度"?"自然日":leave.unit;return '<tr class="attachment-config-row"><td><select class="form-select attachment-basis">'+optionListHtml(["所有申请","本次申请时长","休假日期跨度"],row.basis)+'</select></td><td><span class="attachment-all-text"'+(conditional?' style="display:none"':'')+'>每次申请均需提供</span><div class="attachment-condition"'+(conditional?'':' style="display:none"')+'><select class="form-select attachment-operator">'+optionListHtml(["大于","大于等于"],row.operator||"大于")+'</select><input class="form-input attachment-value" type="number" min="0" value="'+(row.value||"")+'"><span class="attachment-unit">'+unit+'</span></div></td><td>'+attachmentTypeReferenceHtml(row.materials)+'</td><td><select class="form-select attachment-requirement">'+optionListHtml(["全部提供","任意提供一种"],row.requirement||"全部提供")+'</select></td><td><button type="button" class="btn-text remove-attachment-rule">删除</button></td></tr>';}
function attachmentRulesContentHtml(rows,leave){if(!(rows||[]).length){return '<div class="rule-empty-state empty-attachment-rules"><div><span class="rule-empty-badge">待完善</span>已启用附件要求，请至少添加一条附件规则。</div><button type="button" class="btn add-attachment-rule">＋ 添加附件规则</button></div>';}return '<div class="table-wrap"><table class="business-table attachment-rule-table"><thead><tr><th>判断依据 <span class="hint" data-tip="本次申请时长使用系统试算结果；休假日期跨度按开始、结束日期计算自然日。">?</span></th><th>什么时候需要</th><th>需要上传的附件类型</th><th>提交要求</th><th>操作</th></tr></thead><tbody id="attachmentRuleRows">'+rows.map(function(row){return attachmentRuleRowHtml(row,leave);}).join("")+'</tbody></table></div><div class="sub-action"><button type="button" class="btn add-attachment-rule">＋ 添加附件规则</button></div>';}
function unifiedAttachmentHtml(leave,rule){var rows=normalizedAttachmentRules(rule);return businessSectionHtml("9. 附件要求","",'<div id="attachmentRuleBody"'+(rule.attachmentEnabled?'':' style="display:none"')+'>'+attachmentRulesContentHtml(rows,leave)+'</div>',moduleActionHtml("attachmentRuleBody",rule.attachmentEnabled,"请假需要上传材料"));}
function unifiedPolicyHtml(rule){return businessSectionHtml("10. 员工政策说明","员工选择该假别后展示，不参与计算。",'<div id="unifiedPolicyBody"'+(rule.policyEnabled?'':' style="display:none"')+'><span class="form-label"><span class="required">*</span>说明内容</span><textarea class="policy-textarea" id="unifiedPolicyNotice">'+rule.policyNotice+'</textarea></div>',moduleActionHtml("unifiedPolicyBody",rule.policyEnabled,"向员工展示说明"));}
...
function regroupLeaveSections(body){
  ...
  var groups=[
    {title:"1. 基本设置",flat:true,items:[[sections[0],""],[sections[1],""]]},
    {title:"2. 可休时长",items:[[sections[2],"可休时长控制"],[sections[6],"可休时长使用方式"]]},
    {title:"3. 申请规则",items:[[sections[3],"申请资格"],[sections[4],"休假日期"],[sections[5],"申请间隔"],[sections[7],"前置假要求"]]},
    {title:"4. 材料与员工说明",items:[[sections[8],"附件要求"],[sections[9],"员工政策说明"]]}
  ];
  groups.forEach(function(groupData){
    ...
    groupData.items.forEach(function(item){
      ...
      subHead.innerHTML='<div class="business-subsection-title">'+item[1]+'</div>';
...
function openLeaveForm(key){editingLeaveKey=key||"";var leave=key?leaves.find(function(x){return x.key===key;}):{name:"",unit:"小时",min:"1",max:"",calc:"考勤日",roundBase:"0.5",round:"向上取整",daily:"8",prerequisite:"无",strong:"—",paid:"是"};var r=key?JSON.parse(JSON.stringify(unifiedRules[key])):defaultRule();activeLeaveRuleDraft=r;leaveFormDirty=false;document.getElementById("leaveFormTitle").textContent=key?"编辑休假类型－"+leave.name:"新增休假类型";var outline='<div class="form-outline"><button type="button" class="outline-link active" data-section="1">基本设置</button><button type="button" class="outline-link" data-section="2">可休时长</button><button type="button" class="outline-link" data-section="3">申请规则</button><button type="button" class="outline-link" data-section="4">材料与员工说明</button></div>';var body=document.getElementById("leaveFormBody");body.innerHTML=outline+unifiedBaseHtml(leave,!key)+unifiedCalculationHtml(leave)+unifiedEntitlementHtml(key,leave,r)+unifiedApplicationHtml(leave,r)+unifiedLeaveDateHtml(r)+unifiedIntervalHtml(leave,r)+unifiedUsageModeHtml(r)+unifiedOtherHtml(leave)+unifiedAttachmentHtml(leave,r)+unifiedPolicyHtml(r);regroupLeaveSections(body);body.querySelectorAll(".business-section").forEach(function(section,index){section.id="leaveSection"+(index+1);});body.scrollTop=0;document.getElementById("modalMask").classList.add("show");bindUnifiedForm(key,leave,r);body.oninput=markLeaveFormDirty;body.onchange=markLeaveFormDirty;}

function durationRuleDirectFieldsHtml(mode,r,leave){var unit=fieldValue("leaveUnit",(leave&&leave.unit)||"小时");if(mode==="按总可休时长控制"){return businessFieldsHtml([{label:"最多可休",value:r.duration||"",id:"ruleDuration",required:true,suffix:unit}]);}var countMode=r.limitType==="申请次数＋单次时长";return '<div id="rulePeriodAccumulated"'+(countMode?' style="display:none"':'')+'>'+businessFieldsHtml([{label:"周期内最多可休",value:r.duration||"",id:"ruleDuration",required:true,suffix:unit}])+'</div><div id="rulePeriodCount"'+(countMode?'':' style="display:none"')+'>'+businessFieldsHtml([{label:"周期内最多申请次数",value:r.maxTimes||"",id:"ruleMaxTimes",required:true,suffix:"次"},{label:"每次最多可休",value:r.singleDuration||"",id:"ruleSingleDuration",required:true,suffix:unit,tip:"这是每张申请单的上限；与基本信息中的单次最小时长共同决定可提交范围。"}])+'</div>';}
...
function durationConditionEditorHtml(r){var fallback=ruleScopeType(r)==="all";return '<div class="rule-scope-picker"><label class="rule-scope-option"><input type="radio" name="durationRuleScope" value="conditional"'+(fallback?'':' checked')+'>满足以下条件</label><label class="rule-scope-option"><input type="radio" name="durationRuleScope" value="all"'+(fallback?' checked':'')+'>其他情况</label></div><input type="hidden" id="ruleConditionMode" value="常规条件"><div id="ruleStandardConditionPanel"'+(fallback?' style="display:none"':'')+'>'+conditionSetHtml(r.conditions,r.logic||"同时满足全部条件（且）","rule-editor-condition-list")+'</div><div class="field-note" id="fallbackRuleNote"'+(fallback?'':' style="display:none"')+'>当前面的规则都未命中时，使用本条可休结果。</div>';}
...
function ruleResultGroupTitle(mode){if(mode==="按周期限制使用"){return "2. 每个周期内可以怎么使用";}return "2. 符合条件时可以休多少";}
function openDurationRuleEditor(index,key,leave,ruleData){var mode=ruleData.entitlementMode,editorLeave=Object.assign({},leave,{unit:fieldValue("leaveUnit",leave.unit||"小时")}),r=index>=0?JSON.parse(JSON.stringify(ruleData.durationRules[index])):newDurationRuleFromContext(mode,editorLeave,ruleData);r.conditionMode="常规条件";r.resultMode="直接填写";activeDurationRuleEditor={index:index,key:key,leave:editorLeave,ruleData:ruleData,mode:mode,source:r};durationRuleEditorDirty=false;document.getElementById("ruleEditorTitle").textContent=index>=0?"编辑可休时长规则":"新增可休时长规则";document.getElementById("saveDurationRule").textContent="确定";var body=document.getElementById("ruleEditorBody");body.innerHTML='<div class="rule-editor-group"><div class="rule-editor-group-title">时长条件</div><div class="rule-editor-group-desc">选择“满足以下条件”时需配置明确条件；选择“其他情况”时，本条规则必须放在最后。</div><div id="ruleEditorConditions">'+durationConditionEditorHtml(r)+'</div>'+conditionSourceBoxHtml(r)+'</div><div class="rule-editor-group"><div class="rule-editor-group-title">'+ruleResultGroupTitle(mode)+'</div>'+durationRuleResultEditorHtml(mode,r,editorLeave)+'</div>';document.getElementById("ruleEditorMask").classList.add("show");bindDurationRuleEditor();body.oninput=function(){durationRuleEditorDirty=true;};body.onchange=function(){durationRuleEditorDirty=true;};}
...
function bindDurationRuleEditor(){bindDirectChoices(document.getElementById("ruleEditorBody"));document.querySelectorAll('input[name="durationRuleScope"]').forEach(function(input){input.onchange=function(){var fallback=input.value==="all",panel=document.getElementById("ruleStandardConditionPanel"),note=document.getElementById("fallbackRuleNote");if(panel){panel.style.display=fallback?"none":"block";}if(note){note.style.display=fallback?"block":"none";}durationRuleEditorDirty=true;refreshRuleConditionSourceSummary();};});var conditionMode=document.getElementById("ruleConditionMode");document.querySelectorAll('input[name="durationConditionMode"]').forEach(function(input){input.onchange=function(){if(!conditionMode||!input.checked){return;}conditionMode.value=input.value;document.querySelectorAll(".condition-mode-radio").forEach(function(label){label.classList.toggle("active",label.contains(input));});var formula=conditionMode.value==="条件公式";document.getElementById("ruleStandardConditionPanel").style.display=formula?"none":"block";document.getElementById("ruleFormulaConditionPanel").style.display=formula?"block":"none";durationRuleEditorDirty=true;refreshRuleConditionSourceSummary();};});var resultMode=document.getElementById("ruleResultMode");if(resultMode){resultMode.onchange=function(){var formula=resultMode.value==="公式计算";document.getElementById("ruleDirectResult").style.display=formula?"none":"block";document.getElementById("ruleFormulaResult").style.display=formula?"block":"none";refreshPeriodRulePreview();};}var limit=document.getElementById("ruleLimitType");if(limit){limit.onchange=function(){var count=limit.value==="申请次数＋单次时长";var accumulated=document.getElementById("rulePeriodAccumulated"),times=document.getElementById("rulePeriodCount");if(accumulated){accumulated.style.display=count?"none":"block";}if(times){times.style.display=count?"block":"none";}refreshPeriodRulePreview();};}["ruleCycle","ruleDuration","ruleMaxTimes","ruleSingleDuration"].forEach(function(id){var control=document.getElementById(id);if(control){control.addEventListener(control.tagName==="SELECT"?"change":"input",refreshPeriodRulePreview);}});bindConditionRows();bindFormulaEditors();refreshRuleConditionSourceSummary();refreshPeriodRulePreview();}
function collectEditedDurationRule(){var ctx=activeDurationRuleEditor,mode=ctx.mode,source=ctx.source,scopeInput=document.querySelector('input[name="durationRuleScope"]:checked'),scope=scopeInput?scopeInput.value:"conditional",conditions=scope==="conditional"?collectConditionSet(document.querySelector(".rule-editor-condition-list")):[],logic=document.querySelector("#ruleEditorConditions .condition-logic"),r=durationRule(conditions,"",{name:source.name||"",scopeType:scope,conditionMode:"常规条件",conditionFormula:"",conditionSummary:"",kind:durationRuleKind(mode),logic:logic?logic.value:"同时满足全部条件（且）",resultMode:"直接填写",formula:""});r.unit=ctx.leave.unit;if(mode==="按周期限制使用"){r.cycle=fieldValue("ruleCycle","自然月");r.limitType=fieldValue("ruleLimitType","累计时长");if(r.limitType==="申请次数＋单次时长"){r.maxTimes=fieldValue("ruleMaxTimes","");r.singleDuration=fieldValue("ruleSingleDuration","");}else{r.duration=fieldValue("ruleDuration","");}}else{r.duration=fieldValue("ruleDuration","");}return r;}

function resetActiveFormulaCheck(){if(!activeFormulaConfig){return;}activeFormulaConfig.checkStatus="未检测";var status=document.querySelector("#formulaConfigBody .formula-check-status");if(status){status.textContent="未检测";status.className="formula-check-status";}}
...
function bindFormulaTestPanel(panel,editor,expected){var run=panel.querySelector(".formula-test-run");if(!run){return;}run.onclick=function(){var result=panel.querySelector(".formula-test-result"),text;if(expected==="condition"){text='<b>测算结果：符合申请资格</b><br><span>当前示例数据满足公式条件。</span>';}else if(expected==="annual"){var standard=Number((panel.querySelector('[data-sample-key="standard"]')||{}).value||0),active=Number((panel.querySelector('[data-sample-key="active"]')||{}).value||0),total=Number((panel.querySelector('[data-sample-key="total"]')||{}).value||0),amount=total>0?standard*active/total:0;text='<b>测算结果：'+amount.toFixed(2)+'小时</b><br><span>'+standard+' × '+active+' ÷ '+total+' = '+amount.toFixed(2)+'小时</span>';}else{var unit=activeFormulaConfig&&activeFormulaConfig.leave?activeFormulaConfig.leave.unit:"小时";text='<b>测算结果：'+escapeRuleHtml(durationFormulaSampleResult(editor.value))+escapeRuleHtml(unit)+'</b><br><span>结果仅用于验证公式，保存后仍按员工实际数据计算。</span>';}result.innerHTML=text;result.style.display="block";showToast("公式测算完成");};}
...
function bindFormulaEditors(){document.querySelectorAll(".formula-field-button,.formula-token").forEach(function(btn){btn.onclick=function(){insertFormulaToken(btn.dataset.editor,btn.dataset.token);};});document.querySelectorAll(".formula-editor").forEach(function(editor){editor.oninput=function(){resetActiveFormulaCheck();var container=editor.closest(".formula-panel,.annual-drawer-section-body"),panel=container&&container.querySelector(".formula-test-panel");if(panel){panel.classList.remove("show");panel.innerHTML="";}};});document.querySelectorAll(".formula-check").forEach(function(btn){btn.onclick=function(){var scope=btn.closest("#ruleFormulaConditionPanel,#ruleFormulaResult,#formulaConfigBody,.annual-drawer-section-body")||document,editor=scope.querySelector(".formula-editor");if(!editor){return;}editor.classList.remove("input-error");var expected=formulaExpectedType(editor),result=validateFormulaText(editor.value,expected),status=scope.querySelector(".formula-check-status");if(!result.ok){editor.classList.add("input-error");}if(activeFormulaConfig){activeFormulaConfig.checkStatus=result.ok?"通过":"未通过";}if(status){status.textContent=result.ok?"通过":"未通过";status.className="formula-check-status "+(result.ok?"passed":"failed");}showToast(result.message);};});document.querySelectorAll(".formula-test").forEach(function(btn){btn.onclick=function(){var scope=btn.closest("#ruleFormulaConditionPanel,#ruleFormulaResult,#formulaConfigBody,.annual-drawer-section-body")||document,editor=scope.querySelector(".formula-editor");if(!editor){return;}var expected=formulaExpectedType(editor),result=validateFormulaText(editor.value,expected);if(!result.ok){editor.classList.add("input-error");showToast(result.message);return;}editor.classList.remove("input-error");var panel=scope.querySelector(".formula-test-panel");if(!panel){return;}panel.innerHTML=formulaTestPanelHtml(editor,expected);panel.classList.add("show");bindFormulaTestPanel(panel,editor,expected);};});}
...
function bindConditionRows(){
  ...
  document.querySelectorAll(".condition-operator").forEach(function(select){
    select.onchange=function(){
      var row=select.closest(".structured-condition-row"),field=row.querySelector(".condition-field").value,control=row.querySelector(".condition-value"),value=control?(control.classList.contains("gestation-control")?control.dataset.value:control.value):"";
...
function bindDurationRuleSummary(key,leave,ruleData){document.querySelectorAll(".duration-config-option").forEach(function(btn){btn.onclick=function(){var next=btn.dataset.durationMode;if(next===ruleData.durationMode){return;}ruleData.durationMode=next;markLeaveFormDirty();refreshEntitlementDetail(key,leave,ruleData);};});document.querySelectorAll(".edit-duration-summary").forEach(function(btn){btn.onclick=function(){openDurationRuleEditor(Number(btn.dataset.index),key,leave,ruleData);};});document.querySelectorAll(".copy-duration-summary").forEach(function(btn){btn.onclick=function(){var copy=JSON.parse(JSON.stringify(ruleData.durationRules[Number(btn.dataset.index)]));copy.name="";ruleData.durationRules.push(copy);markLeaveFormDirty();refreshEntitlementDetail(key,leave,ruleData);showToast("规则已复制，请修改后确定");};});document.querySelectorAll(".delete-duration-summary").forEach(function(btn){btn.onclick=function(){if(!window.confirm("确定删除这条时长规则吗？")){return;}ruleData.durationRules.splice(Number(btn.dataset.index),1);markLeaveFormDirty();refreshEntitlementDetail(key,leave,ruleData);showToast("规则已删除");};});var draggedIndex=-1;document.querySelectorAll(".duration-summary-row").forEach(function(row){row.ondragstart=function(e){draggedIndex=Number(row.dataset.index);row.classList.add("dragging");e.dataTransfer.effectAllowed="move";};row.ondragover=function(e){e.preventDefault();row.classList.add("drag-over");e.dataTransfer.dropEffect="move";};row.ondragleave=function(){row.classList.remove("drag-over");};row.ondrop=function(e){e.preventDefault();var targetIndex=Number(row.dataset.index);if(draggedIndex<0||targetIndex===draggedIndex){return;}var item=ruleData.durationRules.splice(draggedIndex,1)[0];ruleData.durationRules.splice(targetIndex,0,item);markLeaveFormDirty();refreshEntitlementDetail(key,leave,ruleData);showToast("规则顺序已调整");};row.ondragend=function(){document.querySelectorAll(".duration-summary-row").forEach(function(item){item.classList.remove("dragging","drag-over");});draggedIndex=-1;};});var add=document.querySelector(".add-duration-summary");if(add){add.onclick=function(){openDurationRuleEditor(-1,key,leave,ruleData);};}document.querySelectorAll(".configure-duration-formula").forEach(function(btn){btn.onclick=function(){openDurationFormula(false,leave,ruleData);};});document.querySelectorAll(".view-duration-formula").forEach(function(btn){btn.onclick=function(){openDurationFormula(true,leave,ruleData);};});document.querySelectorAll(".clear-duration-formula").forEach(function(btn){btn.onclick=function(){if(!window.confirm("确定清除统一公式吗？")){return;}ruleData.durationFormula="";ruleData.durationFormulaCheckStatus="未检测";markLeaveFormDirty();refreshEntitlementDetail(key,leave,ruleData);};});}
...
function renderAttachmentTypeOptions(keyword){var query=String(keyword||"").trim(),container=document.getElementById("attachmentTypeOptions"),items=attachmentTypeLibrary.filter(function(item){return item.state!=="停用"&&(!query||item.name.indexOf(query)>-1||item.note.indexOf(query)>-1);});container.innerHTML=items.length?items.map(function(item){return '<label class="attachment-type-option"><input type="checkbox" value="'+item.name+'"'+(pendingAttachmentTypes.indexOf(item.name)>-1?' checked':'')+'><span><div class="attachment-type-name">'+item.name+'</div><div class="attachment-type-note">'+item.note+'</div></span></label>';}).join(""):'<div class="attachment-type-empty" style="padding:20px;text-align:center">未找到已启用的附件类型</div>';container.querySelectorAll('input[type="checkbox"]').forEach(function(input){input.onchange=function(){if(input.checked&&pendingAttachmentTypes.indexOf(input.value)<0){pendingAttachmentTypes.push(input.value);}if(!input.checked){pendingAttachmentTypes=pendingAttachmentTypes.filter(function(item){return item!==input.value;});}};});}
...
function renderAttachmentManager(){var body=document.getElementById("attachmentManagerRows");if(!body){return;}var keyword=fieldValue("attachmentFilterName","").trim(),state=fieldValue("attachmentFilterState","全部"),rows=attachmentTypeLibrary.map(function(item,index){return {item:item,index:index};}).filter(function(row){return (!keyword||row.item.name.indexOf(keyword)>-1||(row.item.note||"").indexOf(keyword)>-1)&&(state==="全部"||row.item.state===state);});document.getElementById("attachmentTypeCount").textContent="共 "+rows.length+" 条";document.getElementById("attachmentPaginationTotal").textContent="共 "+rows.length+" 条";body.innerHTML=rows.length?rows.map(function(row){var item=row.item,index=row.index;return '<tr><td><b>'+escapeRuleHtml(item.name)+'</b></td><td>'+escapeRuleHtml(item.note||"—")+'</td><td><span class="attachment-status'+(item.state==="停用"?' off':'')+'">'+item.state+'</span></td><td>'+escapeRuleHtml(item.updatedBy||"—")+'</td><td>'+escapeRuleHtml(item.updatedAt||"—")+'</td><td><div class="row-actions"><button type="button" class="btn-text edit-attachment-type" data-index="'+index+'">编辑</button><button type="button" class="btn-text toggle-attachment-type" data-index="'+index+'">'+(item.state==="停用"?"启用":"停用")+'</button></div></td></tr>';}).join(""):'<tr><td class="table-empty" colspan="6">没有符合条件的附件类型</td></tr>';body.querySelectorAll(".edit-attachment-type").forEach(function(btn){btn.onclick=function(){openAttachmentTypeEditor(Number(btn.dataset.index));};});body.querySelectorAll(".toggle-attachment-type").forEach(function(btn){btn.onclick=function(){var item=attachmentTypeLibrary[Number(btn.dataset.index)];item.state=item.state==="停用"?"启用":"停用";item.updatedBy="当前用户";item.updatedAt="2026-07-27 10:00";renderAttachmentManager();showToast("附件类型已"+item.state);};});}
...
function openAttachmentTypeEditor(index){editingAttachmentTypeIndex=index;var item=index>=0?attachmentTypeLibrary[index]:{name:"",note:""};document.getElementById("attachmentTypeEditorTitle").textContent=index>=0?"编辑附件类型":"新增附件类型";document.getElementById("attachmentTypeName").value=item.name;document.getElementById("attachmentTypeNote").value=item.note||"";document.getElementById("attachmentManagerMask").classList.add("show");document.getElementById("attachmentTypeName").focus();}
function saveAttachmentTypeEditor(){var name=fieldValue("attachmentTypeName","").trim(),note=fieldValue("attachmentTypeNote","").trim();if(!name){showToast("请填写附件类型名称");return;}var duplicate=attachmentTypeLibrary.findIndex(function(item,index){return item.name===name&&index!==editingAttachmentTypeIndex;});if(duplicate>=0){showToast("附件类型名称不能重复");return;}if(editingAttachmentTypeIndex>=0){Object.assign(attachmentTypeLibrary[editingAttachmentTypeIndex],{name:name,note:note,updatedBy:"当前用户",updatedAt:"2026-07-27 10:00"});}else{attachmentTypeLibrary.unshift({name:name,note:note,state:"启用",updatedBy:"当前用户",updatedAt:"2026-07-27 10:00"});}closeAttachmentManager();renderAttachmentManager();showToast("附件类型已保存");}
...
function bindUnifiedForm(key,leave,ruleData){
  ...
  if(unit){var previousUnit=leave.unit;unit.onchange=function(){
    ...
    if(nextUnit!==previousUnit&&(ruleData.durationRules||[]).length&&!window.confirm("修改计量单位后，已有时长规则将被清空，原数值不会自动换算。确定继续吗？")){this.value=previousUnit;setTimeout(function(){leaveFormDirty=dirtyBefore;},0);return;}
    if(nextUnit!==previousUnit){ruleData.durationRules=[];}
    ...
    ["leaveMin","leaveMax"].forEach(function(id){var input=document.getElementById(id),suffix=input&&input.closest(".input-with-suffix")&&input.closest(".input-with-suffix").querySelector(".input-suffix");if(suffix){suffix.textContent=leave.unit==="天"?"小时":leave.unit;}});
...
function collectConditionSet(container){if(!container){return [];}return Array.from(container.querySelectorAll(":scope > .structured-condition-row")).map(function(row){var control=row.querySelector(".condition-value"),value=control.classList.contains("gestation-control")?control.dataset.value:control.value;return {field:row.querySelector(".condition-field").value,operator:row.querySelector(".condition-operator").value,value:value};}).filter(function(x){return x.field&&(!conditionNeedsValue(x.operator)||x.value!=="");});}
...
function clearLeaveFormErrors(){document.querySelectorAll("#leaveFormBody .input-error").forEach(function(el){el.classList.remove("input-error");});}
...
function validateLeaveForm(ruleData){
  ...
  if(moduleEnabled("attachmentRuleBody")){var attachmentRows=Array.from(document.querySelectorAll(".attachment-config-row"));if(!attachmentRows.length){return "请至少添加一条附件规则";}for(var j=0;j<attachmentRows.length;j++){var row=attachmentRows[j],basis=row.querySelector(".attachment-basis").value;if(basis!=="所有申请"&&!String(row.querySelector(".attachment-value").value||"").trim()){row.querySelector(".attachment-value").classList.add("input-error");return "请填写附件规则的判断数值";}if(!arrayValue(row.querySelector(".attachment-material-values").value).length){return "请为每条附件规则至少选择一种附件类型";}}}
...
function clearRuleEditorErrors(){document.querySelectorAll("#ruleEditorBody .input-error").forEach(function(el){el.classList.remove("input-error");});}
function validateEditedDurationRule(edited){clearRuleEditorErrors();if(edited.scopeType==="conditional"){if(edited.conditionMode==="条件公式"){var conditionFormulaResult=validateFormulaText(edited.conditionFormula,"condition");if(!conditionFormulaResult.ok){markRuleEditorError("#ruleConditionFormula");return conditionFormulaResult.message;}}else{var incomplete=(edited.conditions||[]).some(function(c){return !c.field||!c.operator||(conditionNeedsValue(c.operator)&&!String(c.value||"").trim());});if(incomplete){document.querySelectorAll("#ruleEditorConditions .condition-value").forEach(function(el){if(!String(el.value||"").trim()){el.classList.add("input-error");}});return "请完整填写时长条件";}}}if(edited.resultMode==="公式计算"){var durationFormulaResult=validateFormulaText(edited.formula,"duration");if(!durationFormulaResult.ok){markRuleEditorError("#ruleDurationFormula");return durationFormulaResult.message;}}else if(activeDurationRuleEditor.mode==="按周期限制使用"&&edited.limitType==="申请次数＋单次时长"){if(!String(edited.maxTimes||"").trim()){markRuleEditorError("#ruleMaxTimes");return "请填写周期内最多申请次数";}if(!String(edited.singleDuration||"").trim()){markRuleEditorError("#ruleSingleDuration");return "请填写每次可休时长";}}else if(!String(edited.duration||"").trim()){markRuleEditorError("#ruleDuration");return "请填写可休时长";}var ctx=activeDurationRuleEditor,testRules=ctx.ruleData.durationRules.map(function(r){return JSON.parse(JSON.stringify(r));});if(ctx.index>=0){testRules[ctx.index]=edited;}else{testRules.push(edited);}var result=checkDurationRuleSet(testRules);if(result.level==="error"){return result.text;}return "";}
...
document.getElementById("resetAttachmentFilter").onclick=function(){document.getElementById("attachmentFilterName").value="";document.getElementById("attachmentFilterState").value="全部";renderAttachmentManager();};
...
document.getElementById("saveDurationRule").onclick=function(){if(!activeDurationRuleEditor){return;}var ctx=activeDurationRuleEditor,edited=collectEditedDurationRule(),message=validateEditedDurationRule(edited);if(message){showToast(message);return;}var isEdit=ctx.index>=0;if(isEdit){ctx.ruleData.durationRules[ctx.index]=edited;}else{ctx.ruleData.durationRules.push(edited);}var key=ctx.key,leave=ctx.leave,ruleData=ctx.ruleData;markLeaveFormDirty();closeDurationRuleEditor(true);refreshEntitlementDetail(key,leave,ruleData);showToast(isEdit?"规则修改已保存":"新规则已添加");};
document.getElementById("confirmRules").onclick=function(){var context=document.getElementById("drawer").dataset.context;closeDrawer();showToast(context&&context.indexOf("annual")===0?"年假规则配置已保存":"规则说明已关闭");};
...
document.getElementById("exportPlans").onclick=exportCurrentPlans;
document.getElementById("exportPlanSettings").onclick=openExportPlanSettings;
document.getElementById("saveExportPlanSettings").onclick=saveExportPlanSettings;
document.querySelectorAll("[data-close='export-settings']").forEach(function(x){x.onclick=closeExportPlanSettings;});
document.getElementById("exportSettingsMask").onclick=function(e){if(e.target===this){closeExportPlanSettings();}};
document.getElementById("savePlan").onclick=savePlan;
document.getElementById("cancelPlanEdit").onclick=cancelPlanEditor;
document.querySelectorAll(".plan-edit-field,#planPersonnelTypes input,#planEmployeeTypes input").forEach(function(x){x.oninput=function(){planFormDirty=true;};x.onchange=function(){planFormDirty=true;};});
bindEnumMultiSelect("planPersonnelTypes");bindEnumMultiSelect("planEmployeeTypes");
...
document.getElementById("saveFormulaConfig").onclick=function(){if(!activeFormulaConfig||activeFormulaConfig.readonly){return;}if(activeFormulaConfig.kind==="duration"){var durationValue=fieldValue("durationFormulaDraft","").trim(),formulaRule=activeFormulaConfig.rule,formulaLeave=activeFormulaConfig.leave;formulaRule.durationFormula=durationValue;formulaRule.durationFormulaCheckStatus=activeFormulaConfig.checkStatus||"未检测";markLeaveFormDirty();closeFormulaConfig();refreshEntitlementDetail(editingLeaveKey,formulaLeave,formulaRule);showToast(formulaRule.durationFormulaCheckStatus==="通过"?"可休时长公式已保存":"公式已暂存，检测通过后才能启用方案");return;}var value=fieldValue("qualificationFormulaDraft","").trim();activeLeaveRuleDraft.conditionFormula=value;activeLeaveRuleDraft.conditionFormulaCheckStatus=activeFormulaConfig.checkStatus||"未检测";markLeaveFormDirty();closeFormulaConfig();renderQualificationFormulaSummary();showToast(activeLeaveRuleDraft.conditionFormulaCheckStatus==="通过"?"申请资格公式已保存":"公式已暂存，检测通过后才能启用方案");};
...
renderPlans();renderLeaves();["annual","comp","sick","business"].forEach(renderBoundary);
