...
function openConfirm(title,body,action,options){options=options||{};document.getElementById("confirmTitle").textContent=title;document.getElementById("confirmBody").innerHTML=body;document.getElementById("confirmAction").textContent=options.confirmText||"确定";document.getElementById("cancelConfirm").textContent=options.cancelText||"取消";pendingConfirmAction=action;document.getElementById("confirmMask").classList.add("show");}
function closeConfirm(){document.getElementById("confirmMask").classList.remove("show");document.getElementById("confirmAction").textContent="确定";document.getElementById("cancelConfirm").textContent="取消";pendingConfirmAction=null;}
...
function bindUnifiedForm(key,leave,ruleData){
  ...
  function syncUnitPresentation(){
    ...
    ["leaveMin","leaveMax"].forEach(function(id){
      ...
      if(suffix){suffix.textContent=unit.value;}
      if(field){
        var tip="用于校验一张休假单允许提交的休假"+unit.value+"数。",required=id==="leaveMin"?'<span class="required">*</span>':"";
    ...
    if(summary){summary.textContent=isHour?"员工按小时申请；单次最小/最大时长及日最大时长均按小时校验。":"员工按天申请；单次最小/最大时长按天校验，日最大时长按小时控制每天计入的休假工时。";}
  }
  unit.onchange=function(){
    ...
    if(nextUnit!==previousUnit&&(hasDurationConfig||hasAttachmentThreshold)&&!window.confirm("修改计量单位后，可休时长规则、周期上限及按申请时长设置的附件阈值，需要按新单位重新填写。系统不会自动换算；单次最小/最大时长改用新单位，日最大时长仍按小时保留。是否继续？")){this.value=previousUnit;setTimeout(function(){leaveFormDirty=dirtyBefore;},0);return;}
    if(nextUnit!==previousUnit){ruleData.durationRules=[];ruleData.durationFormula="";ruleData.durationFormulaCheckStatus="未检测";clearUnitDependentAttachmentValues(ruleData);if(nextUnit==="天"){var roundingBase=document.getElementById("roundingBase"),roundingType=document.getElementById("roundingType");if(roundingBase){roundingBase.value="";}if(roundingType){roundingType.value="向上取整";}}}
...
function validateBasicNumericField(id){
  ...
  var value=Number(text),unit=fieldValue("leaveUnit","小时"),message="";
  if(id==="leaveMin"){
    if(!(value>0)){message="单次最小时长必须大于0";}
    else if(unit==="天"&&!Number.isInteger(value)){message="按天计量时，单次最小时长仅支持整数";}
  }else if(id==="leaveMax"){
    ...
    else if(unit==="天"&&!Number.isInteger(value)){message="按天计量时，单次最大时长仅支持整数";}
...
function validateLeaveForm(ruleData){
  ...
  var checks=["leaveTypeSelect","leaveUnit","paidSettingUnified","leaveMin","calcType","dayMax","entitlementMode"];
